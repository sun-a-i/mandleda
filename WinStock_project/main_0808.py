...
class Main(QDialog, main_class):  # param1 = windows : 창,  param2 = ui path
    ...
    def SendOrder(self, accno, type, code, amount, price=0, HogaOrGubun='00'):  # 시장가 매매 TR
        try:
            logger.debug("send order, type:%s, code:%s, amount:%s, price:%s, HoG:%s",type, code, amount, price,HogaOrGubun)
    ...
    def update_table(self):  # 테이블 업데이트 함수
        ...
        try:
            ...
            for i in range(len(self.jango["종목리스트"])):
                ...
                per = int_format(str(round(float(int(self.jango["종목리스트"][i]["현재가"]) / int(self.jango["종목리스트"][i]["매입단가"]) - 1) * 100 , 2)))
