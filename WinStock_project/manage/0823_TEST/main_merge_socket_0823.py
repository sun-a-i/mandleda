...
#CLIENT_HOST = '124.61.26.131'

CLIENT_HOST = '192.168.219.105'
CLIENT_PORT = 5000
        ...
        while self.con == False:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as self.s:
                    ...
                    self.s.bind((CLIENT_HOST, CLIENT_PORT))
                    self.s.listen()
                    self.con = True
                            ...
                            for sock in self.readables:
                                if sock == self.s:  # 신규 클라이언트 접속
                                    ...
                                    try:
                                        ...
                                        logger.debug("클라이언트 접속 해제 : 연결 삭제")
...
class Main(QMainWindow, main_class):  # param1 = windows : 창,  param2 = ui path
    ...
    def __init__(self):
        try:
            ...
            self.socker_server = socket_server_thread()
            self.socker_server.start()
                ...
                if self.cbox_con.currentText() == "000 1":
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        if trade_method == CloseTradeMethod:
                            txt = "종가매매로 시작 : " + self.timeEdit.time().toString()[
    ...
    def update_jango(self):  # 잔고 데이터, 분할매매 데이터 업데이트
        ...
        try:
            ...
            self.table_jango.setItem(1, 0, QTableWidgetItem(format(int(my_cash), ",")))
