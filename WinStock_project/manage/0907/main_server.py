...
class Main(QMainWindow, main_class):  # param1 = windows : 창,  param2 = ui path
    ...
    def __init__(self):
        try:
            ...
            self.test_btn_2.clicked.connect(self.send_test_msg) #메세지 전송 테스트
    ...
    def send_test_msg(self):
        try:
            ...
            self.socket_server.send_all("SELL;171090;1")
            self.real_log_widget.addItem("test_msg_send")
            ...
            if state == 'start':
                if self.cbox_con.currentText() == "000 1": #todo : test용 ?
                    ...
                    if reply == QMessageBox.Yes:
                        ...
                        self.cbox_con_2.setEnabled(False)
                ...
                self.cbox_con_2.setEnabled(True)
    ...
    def condition_refresh(self):
        try:
            ...
            if self.cbox_con_2.currentText() != "":
                logger.debug("조건검색식2 종목 호출")
                logger.debug(self.cbox_con_2.currentText())
                logger.debug(self.con_list)
                con_txt = self.cbox_con_2.currentText()[:3]
    ...
    def set_info(self):
        try:
            ...
            for i in self.con_list:
                self.cbox_con_2.addItem(i + " " + self.con_list[i]["name"])  # 조건검색 목록
    ...
    def div_order_buy(self, state, code):
        ...
        try:
            ...
            if state == 1:
                amt_p = calc_next_price(int(my_cash), -90)  # 가진 금액의 7%
        ...
        try:
            ...
            if test:
                ...
                while sec < CLIENT_WAIT_TIME:
                    sec += 1
                    if sec % 10 == 0:
                        logger.debug("클라이언트 접속 대기중 ..%s", sec)
                    time.sleep(1)
