...
class Main(QMainWindow, main_class):  # param1 = windows : 창,  param2 = ui path
    ...
    def __init__(self):
        try:
            super().__init__()
            self.setupUi(self)
            self.setWindowTitle("WINSTOCK Auto Trading system ver 1.04")
            # ==============키움==============================
    ...
    def trading_state_func(self, state):
        try:
            ...
            if state == 'start':
                ...
                self.trading_start_btn.setStyleSheet("background:gray")
                ...
                self.trading_stop_btn.setStyleSheet("color:white")
                self.trading_stop_btn.setStyleSheet("background:gray")
        except Exception as e:
            logger.debug(e)
            logger.debug(traceback.format_exc())
    ...
    def chk_connecting(self):
        try:
            global auto_flag, chk_img, sock_con_flag
    ...
    def OnReceiveChejanData(self, s_gubun, nItemCnt, s_fid_list):
        try:
            ...
            if s_gubun == '0':
                ...
                nowDate = datetime.now()
                time = str(nowDate.strftime("%H-%M-%S"))
                txt = time + " : " + str(self.GetChejanData("302").strip() + " : " + tmp + " 주문 성공 ")
                logger.debug(txt)
        ...
        try:
            ...
            if amt > 0:
                ...
                if ret == 0:
                    ...
                    self.real_log_widget.addItem("{} 종목 주문 완료".format(code))
                else:
...
HOST_socket = CUSTOM_IP
PORT_socket = 5000
sock_con_flag = False

class socket_client_thread(QThread):
    sock_msg = pyqtSignal(str)
    def __init__(self):
        super().__init__()
        self.con = False
