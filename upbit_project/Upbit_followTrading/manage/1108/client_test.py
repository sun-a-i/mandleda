...
class Main(QMainWindow, main_class):  # param1 = windows : 창,  param2 = ui path
    ...
    def update_table(self, coin_dict, jango_dict):
        try:
            ...
            #logger.debug((    )))
            self.rpb.setValue( (jango_list["total_buy"] / (  float(jango_list["balance"]) + float(jango_list["total_buy"]))) * 100)
    ...
    def coin_chart(self,coin):
        try:
            ...
            # fig, ax = plt.subplots()
            #
            # a = candlestick_ohlc(ax, df_show.values, width=0.0001, colordown='b', colorup='r', alpha=1)
            #
            #
            # ax.grid(True)
            ...
            #self.graphWidget = QGraphicsView()
            #grid_layout = QGridLayout(self.graphWidget)
