    ...
    def auto_order(self, coin):
        try:
            ...
            if list_val != False:
                ...
                coin_data_list[coin_name]['fir_price'] = float(list_val['avg_buy_price'])
                ...
                coin_data_list[coin_name]['next_buy_price'] =calc_next_price(float(coin_data_list[coin_name]['fir_price']), (
                            ...
                            if coin_data_list[coin]['cur_stage'] < coin_data_list[coin]['set_stage']: #set_stage 안에서만
                                if coin['current_price'] < coin_data_list[coin]['next_buy_price'] :# 현재가가 다음 가격보다 작으면
                                    ...
                                    if self.order(1, coin, amt ,is_auto= 1):
                                        ...
                                        coin_data_list[coin_name]['next_buy_price'] = calc_next_price(
                                            float(coin_data_list[coin_name]['fir_price']), (
                                                    float(coin_data_list[coin_name]['div_per']) * float(
                                                coin_data_list[coin_name]['cur_stage'])))


                                        coin_data_list[coin]['gamsi_price'] = calc_next_price(coin_list[coin]['avg_buy_price'], float(coin_data_list[coin]['gamsi_per']))
