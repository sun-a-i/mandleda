...
class Main(QMainWindow, main_class):  # param1 = windows : 창,  param2 = ui path
            ...
            for i in response.json():
                if i['market_warning'] != 'NONE':
                    if i['market'][:3] == 'KRW':
                        ...
                        trade_list[i['market']] = coin_Kname[i['market']]

            logger.debug(f"trade_list = {trade_list}")
    ...
    def add_list_func_all(self):
        try:
            ...
            self.add_listview.clear()
            self.remove_listview.clear()
            trade_list = {k: v for k, v in coin_Kname.items()}
            for i in trade_list:
                self.remove_listview.addItem(i)
    ...
    def remove_list_func(self):
        try:
            ...
            if name not in trade_list.values():
                ...
                for i in coin_Kname.values():
                    if i in trade_list.values():
                        self.remove_listview.addItem(coin_Kname[i])
                    else:
                        self.add_listview.addItem(coin_Kname[i])
    ...
    def coin_chk(self):
        ...
        try:
            ...
            for i in tickers:
                if float(a[i]) < 1: #거래가격이 1원보다 작으면 제외
                    trade_list[i] = coin_Kname[i]
    ...
    def order(self, order_type, coin, amt=0, is_auto=False, is_first=False):
        try:
            global main_upbit
            logger.debug(f"order = {coin}")
            # 일단 날림 !

            # order_type == 1 : buy,  0 : sell
            # order_type; coin; per ; amt; is_auto; is_first
            # 0 or 1; btc.. ; per or pri ; amt; 0 or 1; 0 or 1
            view_txt = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
            coin_data_list[coin]['state'] = 2
            if order_type:
                order_type_txt = "BUY;"
                view_txt += ': ' + str(int(coin_data_list[coin]['cur_stage'])+1) + '차 매수'
            else:
                order_type_txt = "SEL;"
                view_txt += ' : 매도'
    ...
    def auto_order(self, coin):
        try:
            ...
            if auto_flag or test:
                ...
                list_val = False
                if search_mode == OFFENSIVE_MODE:  # defensive 모드가 아니면
                    invest *= 2
                if mycash > invest:
                    if not coin in coin_list:
                        logger.debug('auto_order')
                            ...
                            if coin_data_list[coin]['state'] == 2:  # 프로그램 매수
                                ...
                                if coin_data_list[coin]['cur_stage'] < coin_data_list[coin][
                                    ...
                                    if datetime.datetime.strptime(coin_data_list[coin]['last_trade_time'],
                                        ...
                                        if float(coin_list[coin]['current_price']) < float(coin_data_list[coin][
                                        ...
                                        if coin_data_list[coin]['buy_state']:
                                            # logger.debug('매수 대기중 !')
                                            if float(coin_data_list[coin]['low']) > float(coin_list[coin]['current_price']):
                                                coin_data_list[coin]['low'] = float(coin_list[coin]['current_price'])
                                            ...
                                            if float(coin_data_list[coin]['next_buy_price']) < float(
                                                    coin_list[coin]['current_price']):
                                                amt = coin_data_list[coin]['fir_invest_money'] * (
                                                            2 ** coin_data_list[coin]['cur_stage'])

                                                if float(MyCash) > float(amt):
                                                    logger.debug('next_buy_price')
                                        ...
                                        #self.real_log.addItem("하락방어실행 : " + coin_Kname[coin])
                                        pass
                                if float(coin_list[coin]['current_price']) > float(coin_data_list[coin][
                                ...
                                if coin_data_list[coin]['trailing_state']:  # 감시중이면
                                    # logger.debug('감시중 !')
                                    if float(coin_data_list[coin]['high']) < float(coin_list[coin]['current_price']):
                                        coin_data_list[coin]['high'] = float(coin_list[coin]['current_price'])
                                    ...
                                    if coin_data_list[coin]['trailing_income_price'] > float(
                                            coin_list[coin]['current_price']):
                                        if float(MyCash) > float(amt):
                                            logger.debug('trainiling_income')
    ...
    def state_func(self, state):
        ...
        try:
            ...
            if state == 'start':
                ...
                self.rdo_mode_defensive.setEnabled(False)
                self.rdo_mode_offensive.setEnabled(False)
                ...
                self.rdo_mode_defensive.setEnabled(True)
                self.rdo_mode_offensive.setEnabled(True)
...
def get_ohlcv():
    try:
        ...
        for ticker in tickers:
            #거래량 전처리
            coin_data = pyupbit.get_ohlcv(ticker, 'day', 5)
            time.sleep(0.4)
            value_dict[ticker] = coin_data['value'][-1]
            login_p += 1
            idx += 1
            #전일 고점 대비 등락율
            ohlcv_data[ticker] = coin_data
            ...
            if len(ohlcv_data[ticker]) > 2:
                ...
                if res > 0:
                    ...
                    minus += 1
            logger.debug(f"get ohlcv data : {idx}/{len(tickers)}")
            if ty_test > 1:
                if idx > 10:
                    logger.debug('ty_test 발동중')
                    break
            ...
            try:
                ...
                if auto_flag:
                    if ((datetime.datetime.now().hour == 1) and (datetime.datetime.now().minute == 1)) | \
                        ...
                        value_list = get_ohlcv()
                        time.sleep(60)

                    count = 0
                    #보유종목 20개 이하, 보유 현금 5100원 이상일 경우 체크
                    if (int(my_coin_num) < 20) and (float(MyCash) > 5100):
                        for i in value_list:
                            #거래 제외 종목 제외
                            if i in trade_list:
                                continue
                            ...
                            if len(coin_dic[i]) > 52:
                                ...
                                res = SFI.get_rsi(coin_dic[i], 14, 25)
                                if res == 'ok':
                                    score += 1

                                res = SFI.get_rsi2(coin_dic[i], 14, 20)
                                if res == 'ok':
                                    score += 2
                                ...
                                res = SFI.get_line(coin_dic[i], 5, 3)
                                if res == 'ok':
                                    score += 2

                            buy_flage = False
                            if search_mode == OFFENSIVE_MODE:
                                if score >= 4:
                                    buy_flage = True
                            else:
                                if score >= 6:
                            ...
                            if buy_flage:
                                if score >= 3:
                                    ...
                                    comp_yesterday = float(
                                        calc_income_rate(day_data[i]['today'], coin_dic[i][-1]))
                                    logger.debug("%s %s %s", i, day_data[i]['today'], coin_dic[i][-1])
                                    if comp_yesterday < 5:
                                        if not i in coin_list: #기존 보유 종목 제외
...
def init_coin_dic():
    ...
    try:
        global coin_dic, value_list, coin_data_list, login_p
        tmp_dic = {}
        idx = 0
        for coin in tickers:
        ...
        # 거래량 순위 체크, 당일 시초가 데이터 체크, level 체크
        value_list = get_ohlcv()
...
def update_coin_list():
    try:
        global main_upbit, tickers, current_price_dic, coin_list, MyCash, jango_list, my_coin_num
        ...
        #logger.debug("보유중인 코인 개수 : " +str)
        my_coin_num = len(coin_list.keys())
...
class socket_server_thread(QThread):
    ...
    def run(self):
        try:
            time.sleep(2)
            global SERVER_PORT, main, client_dict
                    ...
                    while True:
                        try:
                            ...
                            for sock in self.readables:
                                logger.debug(f"sock = {sock}")
                                if sock == self.s:  # 신규 클라이언트 접속
                                    newsock, addr = self.s.accept()
                                    logger.debug(f"newsoct = {newsock}, addr = {addr}")
                                    ...
                                    try:
                                        ...
                                        if len(data) > 1:
                                            ...
                                            if data[:2] == "02":
                                                ...
                                                main.client_list.addItem(data + " : 접속")
                                                #client_dict[str(conn.getpeername()[0])] = data
                                                logger.debug("클라이언트 : " + data)

                                                connect_check = '03' + data
                                                conn.sendall(connect_check.encode('utf-8'))
                                            else:
                                                ...
                                                if client_ip in self.clients:
                                                    ...
                                                    main.client_list.addItem(str(name) + " : 해제")
                                                else:
                                                    logger.debug("알수없는 접속 차단 : " + str(sock))
                                                    sock.close()
                                                    self.socks.remove(sock)
                                        else:
                                            logger.debug("알수없는 접속 차단 : " + str(sock))
...
def send_to_clients(txt, view_txt):  #
    try:
        main.socket_server.send_all(txt)
        main.real_log_prt("[send msg] - " + view_txt)
                ...
                if case == 'login':
                    if split_data[0] == 'ok':
                        ...
                        if int(ver) != int(key['ver']):
                            ...
                            """pie = 100 / len(tickers * 2)
                            update_p = str(login_p * pie)"""
...
class MyWindow(QMainWindow, start_class):
    ...
    def login_process(self):
        try:
            ...
            self.update_per.setText(str(round(update_p, 2)) + " / 100")
            #list_men = ['더 나은 버전으로 개선 중입니다.', '안전 투자를 위해 개선 중이니 잠시만 기다려주세요.', '고장난게 아닙니다, 업데이트가 느릴 뿐!', '업데이트가 완료되면 메시지로 알려드립니다.', '지속 발전하는 탼트로 함께 성공 투자!', '탼트는 고급스럽고 값진 프로그램입니다. 아무나 사용하는 로직이 아닙니다!']

            self.sentence.setText("최신 데이터 수집 중...\n잠시만 기다려주세요.")

            if connection_flag != '':
                if connection_flag == 'update':
                    pass
                elif connection_flag == 'get_ohlcv':
                    update_p = login_p * pie
                    #logger.debug(update_p)
                    if update_p > 90:
                        self.update_frame.setVisible(False)
                        connection_flag = ''
                    ...
                    if previous_date < datetime.datetime.now():
                        ...
                        if balances == 'no':
                            ...
                            connection_flag = 'get_ohlcv'
                            self.init_read_thread.start()


                elif connection_flag == 'update_close':
                    self.login_timer.stop()
                    connection_flag = ''
