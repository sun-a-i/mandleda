    ...
    def view_control_func(self, state):
        ...
        try:
            if state == 'start':
                ...
                self.safe_part_ck_2.setEnabled(False)
                ...
                self.safe_part_ck_2.setEnabled(True)
            ...
            if auto_flag:
                ...
                for position in ['long', 'short']:
                    ...
                    if div_data[symbol]['relay']  : #릴레이(체크박스)가 참인지
                        if div_data[symbol][other_position]['relay']: #다른편 릴레이 변수(익절후대기)가 참인지
                            if div_data[symbol][position]['state'] == '대기':  # 대기상태면 매수
                                if self.order_open(div_data[symbol]['start_amt'], div_data[symbol][position]['state'],
                                    ...
                                    div_data[symbol][other_position]['relay'] = False
                                    self.real_log_widget.addItem('릴레이 재구매 완료')
                                    self.real_log_widget.scrollToBottom()

                    if div_data[symbol][position]['safe_part_ck']:
...
#######################################setting 값 관련 함수 ##################################
    ...
    def set_div_data(self):
        ...
        try:
            ...
            div_data[symbol]['short']['safe_part_ck'] = self.safe_part_ck.isChecked()
            div_data[symbol]['short']['safe_roe'] = (float(safe_roe))
            div_data[symbol]['short']['safe_part'] = abs(float(safe_part)/100)

            div_data[symbol]['long']['safe_part_ck'] = self.safe_part_ck_2.isChecked()
